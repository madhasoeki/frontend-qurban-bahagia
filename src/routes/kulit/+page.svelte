<script lang="ts">
	...
	async function handleKulit(id: number, status: 'mulai' | 'selesai') {
		actionLoading[id] = true; actionError[id] = '';
		try {
			await updateProgress(id, 'kulit', status);
			await invalidateAll();
		} catch (err) {
...
<HewanList items={hewan} caption="Daftar Proses Kuliti" actionHeader="Status Kuliti">
	{#snippet cardAction(item)}
		{@const status = getProcessStatus(item.waktuMulaiKuliti, item.waktuSelesaiKuliti)}
...
{#snippet Action(item: ProgressHewan, status: 'idle' | 'running' | 'done')}
	...
	{:else}
		<Badge variant="outline" class="tabular-nums font-mono text-sm">{formatDuration(item.waktuMulaiKuliti!, item.waktuSelesaiKuliti!)}</Badge>
	{/if}
{/snippet}
